...
async function execTmux(
  ...
  if (getPlatform() === 'windows') {
    ...
    // server PID. Same root cause as direct tmux command execution helpers.
 ...
 * Called by tmux-backed tools before initialization.
...
async function doInitialize(): Promise<void> {
  ...
  // created by tmux-backed tools (e.g. 'test', 'verify') would not inherit it.
  ...
  // so later sessions inherit it too. The -e on new-session
